...
public class GameServiceImpl implements GameService {
	...
	// Returns top score for given amount. For example "/topscore/5" will list
	// top 5 scores.
	@Override
	public Collection<Game> getTopScores(int amount, String gameTitle) {
		...
		return this.gameRepository.findByGameTitle(gameTitle).stream()
				.sorted((a, b) -> Integer.compare(b.getScore(), a.getScore())).limit(amount)
				.collect(Collectors.toList());
	}

	// Validate that game file is correct
	@Override
	public boolean validateSubmitScore(GameTemplate submitScore) { // This
																	// validation
																	// can be
																	// improved
																	// as any
	...
	// Update score validation. Only higher is updated, else return the same
	// game object
	@Override
